#!/usr/bin/env bash
# run_server.sh: self-bootstrapping MCP server launcher for apple-eventkit-tools.
#
...
VENV_BIN="${PLUGIN_DATA}/venv/bin/apple-eventkit-mcp"
...
_ENV_FILE="${HOME}/.config/apple-eventkit-tools/env"
if [ -f "${_ENV_FILE}" ]; then
    set -a
