...
Contact data returned by this server isn't necessarily authored by the account owner: contacts can be auto-created from email interactions (`contact_unknown_senders` exists specifically for this), shared/synced from other sources, or proposed by `contact_enrich`'s external lookups. This server also exposes write-capable tools in the same conversation, making it an indirect-prompt-injection vector - the same class closed for mail-tools and sheets-tools. Guarded via the `prompt-security-utils` library - see the root CLAUDE.md's "Prompt injection guarding" section for the shared mechanism. `security_instructions()` is appended after, not replacing, the existing "Always use contact_search before contact_create..." guidance.

**Wrapped fields:**

| Tool(s) | Wrapped fields |
|---------|-----------------|
| `contact_list`, `contact_search` | `name`, `first_name`, `last_name`, `organization`, `job_title`, `emails`/`phones`/`urls[].value` |
| `contact_detail`, `contact_update`, `contact_merge` | the above, plus `notes`, `department`, `addresses[].street/city/state/postal_code/country` |
| `contact_duplicates` | nested contacts (as above), plus `match_value` |
| `contact_incomplete` | `name`, `organization` |
| `contact_last_interaction`, `contact_stale` | `name`, `email` |
| `contact_unknown_senders` | `email`, `name` (raw Mail header data, not even Google-vetted) |
| `contact_enrich`, `contact_enrich_all` | `email`, `name`, plus each `sources[].data` as one wrapped blob per source |

Not wrapped: `emails[].label`/`phones[].label`/`addresses[].label`/`urls[].label` (People API's controlled `type` vocabulary - `_serialize_person()` maps `type` straight to `label` and never reads `customType`, so even a "custom" label is the literal string `"custom"`, not attacker-suppliable text), `id`/`etag`/`has_photo`/`birthday` (opaque or structural, not free text), `contact_groups`' `name` (created via the account owner's own UI action or a predefined system group, never synced from elsewhere), `contact_create`'s response and `contact_add_to_group`/`contact_delete`'s echoed `contact_id`/`group` (freshly created from the caller's own arguments), `contact_export`/`google_people_authorize`/`google_people_status` (no free-text contact content).

**`contact_enrich`'s preview data**: `sources[].data` is freshly-fetched external content (Gravatar, and PeopleDataLabs when `PDL_API_KEY` is set) proposed for the record before - or, when `apply=False`, instead of - ever being written, with a shape that varies per source and no fixed field list to wrap by name. `_wrap_enrichment_sources()` JSON-serializes the whole `data` dict and wraps it as one unit via `wrap_external_data()` (mirrors sheets-tools' `_wrap_values` blob strategy for the same "no fixed shape" reason), regardless of `apply` - the content stays untrusted even after being written, since the write itself doesn't validate or sanitize it.

**Error-path leak (fixed)**: `create_contact()`'s duplicate-name/duplicate-email pre-checks (`google_people.py`) used to interpolate the *existing* conflicting contact's name straight into a `ContactsError` message, which reached the model unwrapped via `contact_create`'s generic exception handler. Rather than wrapping just the name segment of an otherwise-trusted message string, the fix drops the conflicting contact's name from both messages entirely, keeping only its `id`, and points the caller at `contact_search`/`contact_detail` to look it up (where it comes back wrapped). The email-match branch keeps the caller's own `email` argument (their own search input) but drops the matched contact's name the same way.
