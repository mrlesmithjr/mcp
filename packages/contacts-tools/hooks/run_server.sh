#!/usr/bin/env bash
# run_server.sh: self-bootstrapping MCP server launcher for contacts-tools.
#
...
VENV_BIN="${PLUGIN_DATA}/venv/bin/contacts-mcp"
...
_ENV_FILE="${HOME}/.config/contacts-tools/env"
if [ -f "${_ENV_FILE}" ]; then
    set -a
