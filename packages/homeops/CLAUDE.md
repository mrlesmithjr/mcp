...
**Last Updated**: 2026-07-26 (issue #146: `homeops task escalate` replaces the old `claude -p`-based `task-escalation.sh` LaunchAgent script; removed the fictional `homeops remind` command from docs, issue #146 Unit 10)

## Overview

**HomeOps** is a Python CLI + MCP tool for whole-house maintenance management. It tracks recurring tasks, pest control treatments, service providers, maintenance costs, and pushes reminders to iPhone via Apple Reminders.
...
│   ├── reminders.py         # Apple Reminders search-then-create dedup helper (wraps apple_eventkit_tools.RemindersManager, issue #146)
