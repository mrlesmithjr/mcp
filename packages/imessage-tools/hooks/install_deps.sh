    ...
    # using __HOME__; runtime scripts land in ~/.local/share/imessage-tools/.
    #
    ...
    # persisted env file (~/.config/imessage-tools/env), so a user-edited setting
    ...
    if [ -d "${_la_src}" ]; then
        ...
        if [ -f "${_la_src}/render.sh" ]; then
            _LA_ENV_FILE="${HOME}/.config/imessage-tools/env"
            _LA_SCHED_GUARD="${PLUGIN_DATA}/schedule.hash"
            # Guard on the env file existing before hashing it: on a fresh
            # install (no ~/.config/imessage-tools/env yet, before `configure
        ...
        if [ "${_LA_NEEDS_BUILD}" = "1" ]; then
            if [ -d "${_la_src}/scripts" ]; then
                mkdir -p "${HOME}/.local/share/imessage-tools"
                for _s in "${_la_src}"/scripts/*.sh; do
                    [ -e "${_s}" ] && install -m 0755 "${_s}" "${HOME}/.local/share/imessage-tools/$(basename "${_s}")"
                done
            fi
