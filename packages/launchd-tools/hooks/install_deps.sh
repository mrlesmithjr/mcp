    ...
    PLUGIN_DATA="${PLUGIN_DATA:-${CLAUDE_PLUGIN_DATA:-${XDG_DATA_HOME:-${HOME}/.local/share}/launchd-tools}}"
    ...
    # using __HOME__; runtime scripts land in ~/.local/share/launchd-tools/.
    #
    ...
    # persisted env file (~/.config/launchd-tools/env), so a user-edited setting
    ...
    if [ -d "${_la_src}" ]; then
        ...
        if [ "${_LA_NEEDS_BUILD}" = "1" ]; then
            if [ -d "${_la_src}/scripts" ]; then
                mkdir -p "${HOME}/.local/share/launchd-tools"
                for _s in "${_la_src}"/scripts/*.sh; do
                    [ -e "${_s}" ] && install -m 0755 "${_s}" "${HOME}/.local/share/launchd-tools/$(basename "${_s}")"
                done
            fi
