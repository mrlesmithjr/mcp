#!/usr/bin/env bash
# run_server.sh: self-bootstrapping MCP server launcher for launchd-tools.
#
...
VENV_BIN="${PLUGIN_DATA}/venv/bin/launchd-tools-mcp"
...
_ENV_FILE="${HOME}/.config/launchd-tools/env"
if [ -f "${_ENV_FILE}" ]; then
    set -a
