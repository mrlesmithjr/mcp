...
**Status**: ACTIVE
**Last Updated**: 2026-08-01 (issue #39: removed `lawnops/reminders.py` and the `mrlesmithjr-mcp-apple-eventkit-tools` dependency; `lawnops bermuda-check` and `lawnops irrigation check` are now read-only reports, not Apple Reminders creators)
