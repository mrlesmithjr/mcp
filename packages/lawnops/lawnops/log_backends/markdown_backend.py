"""Markdown backend for the lawnops agronomic log (issue #57).
...
configured file (or per-entity files via `lawn_log.markdown.entities`).
Parsing is deterministic and header-driven: the header row is matched
against the entity's canonical column set/order (lawnops.log_schema), and
any row with the wrong column count raises rather than being silently
skipped or reordered. This is deliberately not the freeform regex approach
in lawnops.db.obsidian_import -- that module stays a one-shot legacy
importer; this backend owns a fixed table contract.
...
whole file back (lawnops.atomic_io), so a crash mid-write can never leave a
...
loud, which lawn_log.backend: markdown must never do.
"""
...
from lawnops.log_schema import ENTITIES, EntitySchema
...
# separator on read (issue #57 review MAJOR 3).
...
def _parse_section_raw(content: str, heading: str, spec: EntitySchema) -> list[dict]:
    ...
    if header_cols is None:
        raise RuntimeError(f"Malformed table header under '## {heading}' in lawn_log markdown file")
    if tuple(header_cols) != spec.columns:
        raise RuntimeError(
            f"Column mismatch under '## {heading}': expected {list(spec.columns)}, found {header_cols}. "
            "lawnops owns this table's column set/order when backend: markdown -- fix the header, do not reorder it."
        )
...
def _parse_display_date(raw: str, date_format: str) -> str:
    try:
        ...
        raise RuntimeError(
            f"Date {raw!r} does not match configured lawn_log.markdown.date_format {date_format!r}"
        ) from e
    ...
    for col in spec.columns:
        ...
        if raw in (None, ""):
            ...
            try:
                ...
                raise RuntimeError(f"Non-numeric value {raw!r} in lawn_log column {col!r}") from e
...
class MarkdownBackend:
    def __init__(self, config: dict):
        self.config = config
        self._md_config = (config.get("lawn_log") or {}).get("markdown") or {}
    ...
    def _resolve_path(self, entity: str) -> Path:
        ...
        if not raw_path:
            raise RuntimeError(
                f"lawn_log.backend is 'markdown' but no file path is configured for entity {entity!r}. "
                f"Set lawn_log.markdown.note or lawn_log.markdown.entities.{entity} in config."
            )
    ...
    def _write_rows(self, path: Path, content: str, heading: str, spec: EntitySchema, typed_rows: list[dict]) -> None:
        ...
        # instead of failing loud (issue #57 review MAJOR 2). This still lets
        # a not-yet-existent note FILE be created inside an existing parent
        # directory -- only a missing/non-directory parent is rejected.
        if not path.parent.is_dir():
            raise RuntimeError(
                f"lawn_log markdown parent directory does not exist: {path.parent}. "
                "Refusing to auto-create it -- create the directory (or vault) first, "
                "or fix lawn_log.markdown.note / lawn_log.markdown.entities in config."
            )
        ...
        try:
            ...
            raise RuntimeError(f"Failed writing lawn_log markdown file {path}: {e}") from e
    ...
    def replace_table(self, entity: str, rows: list[dict]) -> int:
        ...
        `lawnops db log-export` migration so re-running it is idempotent
