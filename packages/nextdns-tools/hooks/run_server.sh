#!/usr/bin/env bash
# run_server.sh: self-bootstrapping MCP server launcher for nextdns-tools.
#
...
PLUGIN_DATA="${PLUGIN_DATA:-${CLAUDE_PLUGIN_DATA:-${XDG_DATA_HOME:-${HOME}/.local/share}/nextdns-tools}}"
export PLUGIN_ROOT PLUGIN_DATA
export CLAUDE_PLUGIN_ROOT="${PLUGIN_ROOT}" CLAUDE_PLUGIN_DATA="${PLUGIN_DATA}"

VENV_BIN="${PLUGIN_DATA}/venv/bin/nextdns-mcp"
...
_ENV_FILE="${HOME}/.config/nextdns-tools/env"
if [ -f "${_ENV_FILE}" ]; then
    set -a
