    ...
    PLUGIN_DATA="${PLUGIN_DATA:-${CLAUDE_PLUGIN_DATA:-${XDG_DATA_HOME:-${HOME}/.local/share}/obsidian-search-tools}}"
    ...
    for _cli in "" obsidian-search-tools; do
        [ -z "${_cli}" ] && continue
    ...
    # using __HOME__; runtime scripts land in ~/.local/share/obsidian-search-tools/.
    #
    ...
    # persisted env file (~/.config/obsidian-search-tools/env), so a user-edited setting
    ...
    if [ -d "${_la_src}" ]; then
        ...
        if [ -f "${_la_src}/render.sh" ]; then
            _LA_ENV_FILE="${HOME}/.config/obsidian-search-tools/env"
            _LA_SCHED_GUARD="${PLUGIN_DATA}/schedule.hash"
            # Guard on the env file existing before hashing it: on a fresh
            # install (no ~/.config/obsidian-search-tools/env yet, before `configure
        ...
        if [ "${_LA_NEEDS_BUILD}" = "1" ]; then
            if [ -d "${_la_src}/scripts" ]; then
                mkdir -p "${HOME}/.local/share/obsidian-search-tools"
                for _s in "${_la_src}"/scripts/*.sh; do
                    [ -e "${_s}" ] && install -m 0755 "${_s}" "${HOME}/.local/share/obsidian-search-tools/$(basename "${_s}")"
                done
            fi
    ...
    _CONFIG_DIR="${HOME}/.config/obsidian-search-tools"
    mkdir -p "${_CONFIG_DIR}"
    if [ ! -f "${_CONFIG_DIR}/env" ] && [ ! -f "${_CONFIG_DIR}/env.example" ]; then
        cat > "${_CONFIG_DIR}/env.example" << 'ENVEOF'
# Plugin env vars for obsidian-search-tools.
