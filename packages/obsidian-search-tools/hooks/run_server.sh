#!/usr/bin/env bash
# run_server.sh: self-bootstrapping MCP server launcher for obsidian-search-tools.
#
...
VENV_BIN="${PLUGIN_DATA}/venv/bin/obsidian-search-tools-mcp"
...
_ENV_FILE="${HOME}/.config/obsidian-search-tools/env"
if [ -f "${_ENV_FILE}" ]; then
    set -a
