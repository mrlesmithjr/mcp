#!/usr/bin/env bash
# run_server.sh: self-bootstrapping MCP server launcher for weather-tools.
#
...
VENV_BIN="${PLUGIN_DATA}/venv/bin/weather-tools-mcp"
...
_ENV_FILE="${HOME}/.config/weather-tools/env"
if [ -f "${_ENV_FILE}" ]; then
    set -a
