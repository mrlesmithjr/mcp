    ...
    PLUGIN_DATA="${PLUGIN_DATA:-${CLAUDE_PLUGIN_DATA:-${XDG_DATA_HOME:-${HOME}/.local/share}/ynab-tools}}"
    ...
    if [ "${NEEDS_BUILD}" = "1" ]; then
        ...
        if [ -f "${GUARD}" ] && [ "$(cat "${GUARD}")" = "${CURRENT_HASH}" ]; then
            ...
            uv pip install --python "${VENV_TMP}/bin/python" "${PLUGIN_ROOT}[dashboard]"
    ...
    for _cli in "" ynab ynab-dashboard; do
        [ -z "${_cli}" ] && continue
    ...
    # using __HOME__; runtime scripts land in ~/.local/share/ynab-tools/.
    #
    ...
    # persisted env file (~/.config/ynab-tools/env), so a user-edited setting
    ...
    if [ -d "${_la_src}" ]; then
        ...
        if [ "${_LA_NEEDS_BUILD}" = "1" ]; then
            if [ -d "${_la_src}/scripts" ]; then
                mkdir -p "${HOME}/.local/share/ynab-tools"
                for _s in "${_la_src}"/scripts/*.sh; do
                    [ -e "${_s}" ] && install -m 0755 "${_s}" "${HOME}/.local/share/ynab-tools/$(basename "${_s}")"
                done
            fi
